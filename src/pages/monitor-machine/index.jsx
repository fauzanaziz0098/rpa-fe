...
export default function Home({ headers }) {
  ...
  useEffect(() => {
    if (activePlan?.id) {
      const fetchData = async () => {
        try {
          const res = await axiosHour.get(
            `production/data-active/${activePlan.id}`,
            getHeaderConfigAxios()
          );
      ...
      const fetchNoPlanMachineByShiftToday = async () => {
        const res2 = await axiosPlanning.get(
          `no-plan-machine/findOneByShift/${activePlan.shift.id}`,
          getHeaderConfigAxios()
        );
  ...
  const totalActual = () => {
    let total = 0;
    productionData.map((item) => (total += item.qty_actual));
  ...
  const totalTarget = () => {
    ...
    let totalTarget = 0;
    {
      productionData.map((item, index) => {
        ...
        noPlanToday.map((row) => {
          if (
            moment(row.time_in, "HH:mm").isSameOrAfter(
              moment(item.time_start, "HH:mm")
            ) &&
            moment(row.time_in, "HH:mm").isSameOrBefore(
              moment(item.time, "HH:mm")
            ) &&
            moment(row.time_out, "HH:mm").isSameOrAfter(
              moment(item.time_start, "HH:mm")
            ) &&
            moment(row.time_out, "HH:mm").isSameOrBefore(
              moment(item.time, "HH:mm")
            )
          ) {
            ...
            moment(row.time_out, "HH:mm").isAfter(
              moment(item.time_start, "HH:mm")
            )
          ) {
            noPlanTotal +=
              moment(row.time_out, "HH:mm").diff(
                moment(item.time, "HH:mm"),
                "minutes"
              ) - 1;
          } else if (
            moment(row.time_in, "HH:mm").isBefore(
              moment(item.time_start, "HH:mm")
            ) &&
            moment(row.time_out, "HH:mm").isSameOrAfter(
              moment(item.time_start, "HH:mm")
            ...
            noPlanTotal += moment(row.time_out, "HH:mm").diff(
              moment(item.time_start, "HH:mm"),
              "minutes"
            );
          }
        });
        totalTarget += Math.round(
          activePlan?.qty_per_minute * (60 - noPlanTotal)
        );
      });
    }
    return totalTarget;
  ...
  const durasiMqtt = () => {
    ...
    if (productionData.length == 0) {
      duration = 60 - moment(activePlan.date_time_in, "HH:mm").minute();
      timeStart = moment(activePlan?.date_time_in)
        .tz("Asia/Bangkok")
        .format("HH:mm");
      timeEnd =
        moment(activePlan.date_time_in)
          .tz("Asia/Bangkok")
          .add(1, "hour")
          .format("HH") + ":00";
    } else {
      timeStart = moment(
        productionData[productionData.length - 1]?.time_start,
        "HH:mm"
      )
        .add(1, "hours")
        .format("HH:mm");
    ...
    noPlanToday.map((row) => {
      if (
        ...
        moment(row.time_in, "HH:mm").isSameOrBefore(moment(timeEnd, "HH:mm")) &&
        moment(row.time_out, "HH:mm").isSameOrAfter(
          moment(timeStart, "HH:mm")
        ) &&
        moment(row.time_out, "HH:mm").isSameOrBefore(moment(timeEnd, "HH:mm"))
        ...
        moment(row.time_in, "HH:mm").isBefore(moment(timeStart, "HH:mm")) &&
        moment(row.time_out, "HH:mm").isSameOrAfter(
          moment(timeStart, "HH:mm")
        ) &&
        moment(row.time_out, "HH:mm").isSameOrBefore(moment(timeEnd, "HH:mm"))
      ) {
        duration -= moment(row.time_out, "HH:mm").diff(
          moment(timeStart, "HH:mm"),
          "minutes"
        );
      }
  ...
  const totalPersentase =
    productionData.length == 0
      ? Math.round((qtyActualMqtt() / qtyTargetMqtt()) * 100)
      : Math.round(
          isNaN(totalActual() / totalTarget())
            ? 0
            : ((totalActual() + qtyActualMqtt()) /
                (totalTarget() + qtyTargetMqtt())) *
                100
        );

  const timeEstimation =
    activePlan?.qty_planning * activePlan?.product?.cycle_time;
  const estimation = moment(activePlan.date_time_in)
    .tz("Asia/Bangkok")
    .add(timeEstimation, "minute")
    .format("HH:mm");
                  ...
                  >
                    {productionData.map((item, index) => {
                      ...
                      if (index == 0) {
                        duration = duration =
                          60 -
                          moment(activePlan.date_time_in, "HH:mm").minute();
                      }
                      noPlanToday.map((row) => {
                        if (
                          moment(row.time_in, "HH:mm").isSameOrAfter(
                ...
                <div>
                    ...
                    >
                      {productionData.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            {Math.round(
                              (60 - noPlanTotal) * activePlan.qty_per_minute
                            )}
                          </p>
                        );
                      })}
                  ...
                  >
                    ...
                    >
                      ...
                      <p style={{ width: "100px" }}>
                        {mqttData1.qty_actual ? mqttData1.qty_actual : 0}
                      </p>
                    </div>
                  </div>
                  <div
                    ...
                    >
                      ...
                      <p
                        style={{
                          width: "50px",
                          textAlign: "center",
                          color: `${persentaseMqtt() >= 100 ? "green" : ""}`,
                        }}
                      >
                        {persentaseMqtt()}%
                      </p>
                      <p
                        style={{
                          width: "100px",
                          color: `${totalPersentase >= 100 ? "green" : ""}`,
                        }}
                      >
                        {totalPersentase}%
                      </p>
