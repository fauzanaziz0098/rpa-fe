    ...
    useEffect(() => {
      if (activePlan?.id) {
          const fetchData = async () => {
              try {
                  const res = await axiosHour.get(`production/data-active-new/${activePlan.id}`, getHeaderConfigAxios())
      ...
      if (activePlan.id) {
          setInterval(async () => {
              try {
                  ...
                  const res = await axiosHour.get(`production/data-active-new/${activePlan.id}`, headers)
                  setProductionData(res.data.data)
              } catch (error) {
                            ...
                            <div>
                                <div style={{ display: 'flex', marginTop: '10px', fontWeight: 'bold' }}>
                                    ...
                                    {productionData?.all?.map((value, key) => {
                                        return (
                                        <p key={key} style={{width: '50px', textAlign: 'center' }}>{value.time}</p>
                                        )
                                    })}
                                ...
                                {productionData?.all?.map((value, key) => {
                                    return (
                                        <p key={key} style={{width: '50px', textAlign: 'center' }}>{value.duration}</p>
                                    )
                                    })}
                                    <p style={{width: '100px' }}>{productionData?.all?.reduce((total, value) => total + value.duration, 0)}</p>
                                </div>
                            </div>
                        </div>
                        <div
                            ...
                            <div>
                                <div
                                    ...
                                    {productionData?.all?.map((value, key) => {
                                    return (
                                        <p key={key} style={{ width: '50px', textAlign: 'center' }}>{value.target}</p>
                                    )
                                    })}

                                        <p style={{  width: '100px' }}>{productionData?.all?.reduce((total, value) => total + value.target, 0)}</p>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', fontSize: '35px', fontWeight: 'bold', color: 'gold' }}>
                                    ...
                                    {productionData?.all?.map((value, key) => {
                                    return (
                                        <p key={key} style={{ width: '50px', textAlign: 'center' }}>
                                        {value.time.split(':')[0] == new Date().getHours()
                                            ? mqttData1.qty_actual - productionData.all.reduce((total, value) => total + value.actual,0)
                                            : value.actual}
                                        </p>
                                    )
                                    })}

                                        <p style={{  width: '100px' }}>{Number(mqttData1.qty_actual)}</p>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', fontSize: '22px', fontWeight: 'bold', color: 'firebrick' }}>
                                    ...
                                    {productionData?.all?.map((value, key) => {
                                    return (
                                        <p key={key} style={{ width: '50px', textAlign: 'center' , color: `${
                                        (value.time.split(':')[0] == new Date().getHours()
                                        ? Math.round((mqttData1.qty_actual - productionData.all.reduce((total, value) => total + value.actual,0)) / value.target * 100)
                                        : value.percentage) >= 100 ? 'green': ''}` }}>
                                            {value.time.split(':')[0] == new Date().getHours()
                                            ? Math.round((mqttData1.qty_actual - productionData.all.reduce((total, value) => total + value.actual,0)) / value.target * 100)
                                            : value.percentage}%
                                        </p>
                                        ...
                                        <p style={{  width: '100px', color: `${
                                            (Math.round((Number(mqttData1.qty_actual)) / productionData?.all?.reduce((total, value) => total + value.target, 0) * 100)) >= 100 ? 'green' : ''}` }}>{Math.round(Number(mqttData1.qty_actual) / productionData?.all?.reduce((total, value) => total + value.target, 0) * 100)}%</p>
                                    </div>
                                </div>
                            </div>
                        </div>
